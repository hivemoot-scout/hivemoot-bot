...
1. Health endpoint: open `/api/github/webhooks` on your deployment. A healthy bot returns JSON with `"status":"ok"`. `"status":"misconfigured"` or HTTP `503` means the app is missing required setup, and the response includes a `missing` array naming the missing variables such as `APP_ID`, `PRIVATE_KEY`/`APP_PRIVATE_KEY`, or `WEBHOOK_SECRET`.
